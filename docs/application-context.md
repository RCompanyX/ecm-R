...
After the user explicitly approves the conversational plan, `ecmr-plan` runs the OpenSpec propose workflow in the same planning session. For workflow-only changes with no spec delta, it runs `openspec new change "<name>"` without `--skip-specs`; OpenSpec 1.12.0 supports that option for `archive`, not `new change`. It then runs `openspec status --change "<name>" --json`, takes the CLI-returned `changeRoot`, and adds only `skip_specs: true` to the CLI-created `.openspec.yaml`, preserving `schema`, `created`, store context, and all other metadata. A second status check must report `specs` as `skipped` before proposal, design, or tasks are created, and no specs file is created. Product changes retain the `specs` artifact and do not receive the marker. For every remaining artifact, the planner follows status-reported dependency edges, instructions, templates, and `resolvedOutputPath`, re-running status after each write; it validates with `openspec validate "<name>" --type change --strict` (and `openspec validate --all --strict` when required). It writes only resolved OpenSpec planning artifacts under `openspec/changes/` (and main specs under `openspec/specs/` when running `/opsx-sync`), reports `ARTIFACTS_READY`, and stops. It never delegates to `ecmr-dev` or treats artifact readiness as implementation approval.
